...
DISTRIBUTION = 6 if not arg[1] else arg[1]
...
for f in os.listdir("./datasets/{}/all_data".format(platform)):
    if f.find(".gui") != -1:
        ...
        image_path = "./datasets/{}/all_data/{}.png".format(platform,f)
        ...
        np.savez_compressed("./datasets/{}/all_data/{}".format(platform, f),
...
for index, set in enumerate([val_data, test_data, train_data]):
    for file_name in set:
        shutil.copyfile("./datasets/{}/{}/{}.npz".format(platform, search_index[index], file_name),
